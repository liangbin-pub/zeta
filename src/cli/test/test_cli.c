...
{
	rpc_trn_endpoint_t *ep = (rpc_trn_endpoint_t *)value;
	rpc_trn_endpoint_t *c_ep = (rpc_trn_endpoint_t *)check_value_data;

	assert_memory_equal(ep->mac, c_ep->mac, sizeof(char) * 6);

	assert_memory_equal(ep->mac, c_ep->mac, sizeof(char) * 6);

	assert_int_equal(ep->vni, c_ep->vni);

	assert_int_equal(ep->ip, c_ep->ip);

	assert_int_equal(ep->hip, c_ep->hip);
			  ...
			  const LargestIntegralType check_value_data)
{
	...
	rpc_trn_endpoint_t *ep = epb->rpc_trn_endpoint_batch_t_val;
	rpc_trn_endpoint_t *c_ep = c_epb->rpc_trn_endpoint_batch_t_val;
			      ...
			      const LargestIntegralType check_value_data)
{
	rpc_trn_endpoint_key_t *ep_key =
		(rpc_trn_endpoint_key_t *)value;
	rpc_trn_endpoint_key_t *c_ep_key =
		(rpc_trn_endpoint_key_t *)check_value_data;
...
static void test_trn_cli_update_ep_subcmd(void **state)
{
	...
	memcpy(ep.mac, mac, sizeof(char) * 6);
	memcpy(ep.hmac, hmac, sizeof(char) * 6);

	rpc_trn_endpoint_batch_t exp_epb = {
		.rpc_trn_endpoint_batch_t_len = 1,
		.rpc_trn_endpoint_batch_t_val = &ep,
	};
...
static void test_trn_cli_get_ep_subcmd(void **state)
{
	...
	rpc_trn_endpoint_t get_ep_1_ret_val = {
		.vni = 3,
		.ip = 0x100000a,
		.hip = 0x200000a,
	};
	memcpy(get_ep_1_ret_val.mac, mac, sizeof(char) * 6);
	memcpy(get_ep_1_ret_val.hmac, hmac, sizeof(char) * 6);
	...
	rpc_trn_endpoint_key_t exp_ep_key = {
		.ip = 0x100000a,
		.vni = 3,
	};
