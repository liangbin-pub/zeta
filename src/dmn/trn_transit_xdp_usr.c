...
static trn_xdp_prog_t trn_prog_tbl[TRAN_MAX_PROG] = {
	...
	{TRAN_FWD_PROG, "/trn_xdp/trn_transit_fwd_proc_xdp_ebpf.o"},
	{TRAN_FTN_PROG, "/trn_xdp/trn_transit_ftn_proc_xdp_ebpf.o"},
};
...
static trn_xdp_prog_t trn_prog_dbg_tbl[TRAN_MAX_PROG] = {
	...
	{TRAN_FWD_PROG, "/trn_xdp/trn_transit_fwd_proc_xdp_ebpf_debug.o"},
	{TRAN_FTN_PROG, "/trn_xdp/trn_transit_ftn_proc_xdp_ebpf_debug.o"},
};
...
static trn_xdp_map_t trn_xdp_bpfmaps[] = {
	...
	{"hosted_endpoints_iface_map", true, -1, NULL},
	{"interface_config_map", true, -1, NULL},
	{"interfaces_map", true, -1, NULL},
	{"zgc_entrances_map", true, -1, NULL},
	{"fwd_flow_mod_cache", true, -1, NULL},
	{"rev_flow_mod_cache", true, -1, NULL},
	{"ep_flow_host_cache", true, -1, NULL},
...
#undef TRN_LOG_INFO
#undef TRN_LOG_ERROR
#define TRN_LOG_ERROR printf
#define TRN_LOG_INFO printf
#endif
...
static int trn_transit_map_hash(void)
{
	...
	hcreate(num_maps);
	for (int i = 0; i < num_maps; i++) {
        e.key = strdup(trn_xdp_bpfmaps[i].name);  // key needs to be malloced when insert
        e.data = (void *) &trn_xdp_bpfmaps[i];
        ep = hsearch(e, ENTER);
        /* there should be no failures */
        if (ep == NULL) {
            TRN_LOG_ERROR("Failed to hashing bpfmap %s, destroy hash", e.key);
			hdestroy();
            return 1;
        }
		
		trn_xdp_map_t *xdpmap = trn_transit_map_get(trn_xdp_bpfmaps[i].name);
		if (!xdpmap || xdpmap != e.data) {
            TRN_LOG_ERROR("Failed to find bpfmap after hashing %s, destroy hash", e.key);
			hdestroy();
            return 1;
		} else {
            TRN_LOG_ERROR("Successfully hashed bpfmap %s", e.key);
		}
    }
#endif	
	return 0;
	...
	bpf_object__for_each_map(map, obj) {
		...
		if (xdpmap->shared) {
			...
			if (fd >= 0) {
				...
				close(fd);
				TRN_LOG_INFO("Will reuse pinned map %s.\n",
					map_name);
			} else {
				TRN_LOG_INFO("Will pin %s map for reuse.\n",
					map_name);
			}
		}		
	}
	...
	bpf_object__for_each_map(map, obj) {
		...
		if (xdpmap->shared) {
			...
			if (fd >= 0)
			{
				...
				if (bpf_map__pin(map, buf)) {
					TRN_LOG_ERROR("Error pinning map %s.\n", map_name);
					return 1;
				}
				xdpmap->fd = bpf_map__fd(map);
				TRN_LOG_INFO("POST: successfully pinned map %s for reuse.\n",
					map_name);
			}
		} else if (prog_idx == TRAN_TRANSIT_PROG &&
			!strcmp(map_name, "xdpcap_hook_map")) {
			/* pin but not shared */
			if (bpf_map__unpin(map, prog->pcapfile) == 0) {
				TRN_LOG_INFO("unpin exiting pcap map file: %s", prog->pcapfile);
			}

			if (bpf_map__pin(map, prog->pcapfile)) {
				TRN_LOG_ERROR("Failed to pin xdpcap map to %s", prog->pcapfile);
				return 1;
			}
		}	
...
static int trn_transit_xdp_map_initialize(void)
{
	...
	/* Don't initialize interface_config_map untill droplets created */
...
int trn_update_endpoint(struct endpoint_key_t *epkey, struct endpoint_t *ep)
{
	int fd, err;
...
int trn_get_endpoint(struct endpoint_key_t *epkey, struct endpoint_t *ep)
{
	int fd, err;
...
int trn_delete_endpoint(struct endpoint_key_t *epkey)
{
...
{
	int	fd = trn_transit_map_get_fd("interface_config_map");
	if (fd < 0) {
		TRN_LOG_ERROR("Failed to get interface_config_map fd");
		return 1;
	}

	int err = bpf_map_update_elem(fd, &itf->iface_index, itf, 0);
	if (err) {
		TRN_LOG_ERROR("Failed to update interface_config_map: %d (err:%d).",
			itf->iface_index, err);
		return 1;
	}
...
// parameters: 
int trn_transit_xdp_load(char **interfaces, bool debug)
{
	int i;
...
#if 0
		if (trn_get_interface_ipv4(eth->iface_index, &eth->ip)) {
			TRN_LOG_ERROR("Failed to get IP address for %s", interfaces[i]);
			goto cleanup;
		}
		if (trn_get_interface_mac(eth->iface_index, &eth->mac[0])) {
			TRN_LOG_ERROR("Failed to get MAC address for %s", interfaces[i]);
			goto cleanup;
		}
#endif
		prog = &md->objs[i].xdp;
