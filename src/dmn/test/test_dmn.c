...
static struct test_bpf_map_t maps[] = {
	...
	{.name="ftns_map"},
	{.name="endpoints_map"},
	{.name="hosted_endpoints_iface_map"},
	{.name="interface_config_map"},
	{.name="interfaces_map"},
	{.name="zgc_entrances_map"},
	{.name="fwd_flow_mod_cache"},
	{.name="rev_flow_mod_cache"},
	{.name="ep_flow_host_cache"},
	{.name="ep_host_cache"},
	{.name="xdpcap_hook"},
};
...
static int check_ep_equal(rpc_trn_endpoint_t *ep, rpc_trn_endpoint_t *c_ep)
{
	...
	assert_memory_equal(ep->hmac, c_ep->hmac, sizeof(char) * 6);
...
static void test_update_ep_1_svc(void **state)
{
	...
	memcpy(ep1.hmac, hmac, sizeof(char) * 6);

	rpc_trn_endpoint_batch_t epb = {
		.rpc_trn_endpoint_batch_t_len = 1,
		.rpc_trn_endpoint_batch_t_val = &ep1,
	};
...
static void test_get_ep_1_svc(void **state)
{
	...
	memcpy(ep_val.hmac, ep1.hmac, sizeof(ep1.hmac));
