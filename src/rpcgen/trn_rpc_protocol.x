...
typedef string intf_name<TRAN_MAX_ITF_SIZE>; 
...
struct addr_t {
	string ip<16>;          /* IP address */
	string mac<18>;         /* MAC address */
};
...
struct rpc_trn_droplet_t {
       intf_name interface;
       uint8_t role;
       uint8_t num_entrances;
       struct addr_t entrances[TRAN_MAX_ZGC_ENTRANCES];
};

/* Defines interfaces for xdp prog to attach/detatch */
struct rpc_trn_xdp_intf_t {
       intf_name interfaces[TRAN_ITF_MAP_MAX];
       uint32_t debug_mode;
};
...
program RPC_TRANSIT_REMOTE_PROTOCOL {
        version RPC_TRANSIT_ALFAZERO {
                ...
                rpc_trn_endpoint_t GET_EP(rpc_trn_endpoint_key_t) = 7;
